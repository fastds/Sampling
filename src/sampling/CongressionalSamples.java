...
public class CongressionalSamples {
	//定义与操作的表相关的信息
	...
	private List<Float> scaleFactor;
	public CongressionalSamples(){
		//读取配置文件，获取相关表信息
		Properties prop = new Properties();
		try {
			prop.load(CongressionalSamples.class.getResourceAsStream("/TableInfo.properties"));
		...
		this.scaleFactor = new ArrayList<Float>();
		init();
	}
	/**
	 * 1、查询数据库，基于分组属性进行分组
			...
			try {
				...
				while(rs.next()){
					...
					for(Entry<Integer, List<String>> entry : this.groupingValues.entrySet()){
						...
						if(equalFlag){
							System.out.println("sub:"+entry.getValue()+",parent:"+rs.getString(1)+",ratio:"+(float)this.tupleNumOfGrouping.get(entry.getKey())/parentNum);
	 ...
	 * 对原始数据进行取样
	 * 将原始数据存储在数据库当中
	 * 样本表：原始表名_sample(rowID, 原始属性集, scale factor)
	 */
